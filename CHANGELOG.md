...
없음. `claude-code-kit` 은 `v2.21.0` 으로 마감됐고 개발은 후속 킷
...
### 함께 실린 변경 — 태그 이전에 출하됐으나 릴리스 노트에 접히지 않았던 것 (2026-09-08)

아래는 `v2.21.0` 태그(`37c15ba`) **이전** 커밋 — `7520b24`(비공개 프로젝트명 제거 +
gitleaks 룰 실패 명시)·`20bdfe7`(CI red 수정), 둘 다 2026-09-08 — 로 이미 출하됐는데
`[Unreleased]` 에 남아 있던 항목이다. 이 흡수는 2026-09-10 의 사후 편집이다. 최종본의 문서 정합을 맞추려고 이 릴리스 항목으로 흡수했다 —
**코드 변경은 없다.** 전부 문서·스캐너 설정·리포지토리 게이트·eval 자산이고
`plugins/` 런타임은 건드리지 않으므로, 위 *"이 릴리스는 버그 수정 하나로 한정한다"* 는
배포되는 플러그인 동작 기준으로 그대로 참이다.
