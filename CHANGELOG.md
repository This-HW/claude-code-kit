...
**왜 폐기했나.** 커버리지를 13종으로 넓히자(W-018) 관측 8종 중 6종이 이 마커를
간헐적으로 생략한다는 게 드러났다. 판별 결과(W-021) hooks·skills·scripts·rules
어디에도 이 블록을 파싱하는 결정론적 코드가 없었다 — 신호는 아무도 읽지 않는
사문화된 2차 경로였는데도 파이프라인은 정상 동작했다. (측정 방법론에 교란이 있었다는
후속 정정도 있다 — `implement-code`의 안정적 준수는 `task.md`가 신호 보고를 직접
지시한 조건의 결과였다. 다만 폐기 근거는 이 통계가 아니라 결정론적 파싱 부재와
