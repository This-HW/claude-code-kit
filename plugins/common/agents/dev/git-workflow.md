...
채택할지는 코드 소유자의 판단이 필요한 결정이다. 충돌 파일과 내용을 보고하고 해결
방안을 제시한 뒤 **사용자 결정을 기다린다** (출력 형식은 아래 "충돌 발생 시" 참고).

**무인 실행/미응답 시 안전 기본값은 되돌리기다.** 사용자 결정을 받을 수 없는 상황(예:
비대화형 실행)에서는 해결하고 진행하는 것이 아니라 아래처럼 **되돌리고 보고**한다 —
임의로 골라 진행하는 쪽이 기본값이 되어서는 안 된다.
...
# 머지 중단 (되돌리기 — 사용자 결정을 받을 수 없을 때의 기본 동작)
git merge --abort
...
절차다 — 이 에이전트가 먼저 실행하지 않는다:

```bash
# (사용자가 선택한 방식으로 충돌을 해결한 뒤) 해결 완료 표시 후 계속
